...
class MeshConvolution(tf.Module):
    ...
    def __init__(self, out_channels, k=5, bias=True):
        ...
        self.conv = tf.keras.layers.Conv2D(
            filters=out_channels, kernel_size=(1, k), bias_initializer=bias
        )
        self.k = k
    ...
    def call(self, edge_feats, mesh):
        ...
        G = tf.concat(
            [self.pad_neighbors(i, edge_feats.shape[2]) for i in mesh], axis=0
        )
    ...
    def create_neighbors(self, edge_feats, G):
        ...
        f = tf.gather_nd(edge_feats, G_flat)
    ...
    def pad_neighbors(self, mesh, size):
        ...
        padded_neighbors = tf.concat(
            (tf.expand_dims(tf.range(len(mesh.edges)).float(), 1), padded_neighbors),
            axis=1,
        )
        padded_neighbors = tf.pad(
            padded_neighbors, [0, 0, 0, size - len(mesh.edges)], "CONSTANT"
        )
